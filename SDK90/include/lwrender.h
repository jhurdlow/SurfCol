...
typedef double		 LWRayTraceNewFunc (const LWDVector position,
					 const LWDVector direction,
					 LWDVector color,
					 const double eta,
					 const int mode);
...
typedef double		 LWRayShadeFunc (const LWDVector position,
					 const LWDVector direction,
					 struct st_LWShaderAccess *access);
 ...
 * Globals.
 */
#define LWITEMINFO_GLOBAL	"LW Item Info 4"
...
#define LWOBJECTINFO_GLOBAL	"LW Object Info 7"
...
typedef struct st_LWObjectInfo {
	...
	double (*morphAmount)( LWItemID objectitem, LWTime time);
	unsigned int (*edgeOpts)( LWItemID objectitem);
	void (*edgeColor)( LWItemID objectitem, LWTime time, LWDVector color);
	int (*subdivOrder)( LWItemID objectitem);
	double (*polygonSize)( LWItemID objectitem, LWTime time);
	int (*excluded)( LWItemID objectitem, LWItemID lightitem);
	void (*matteColor)( LWItemID objectitem, LWTime time, LWDVector color);
	double (*thickness)( LWItemID objectitem, LWTime time, int type);
	double (*edgeZScale)( LWItemID objectitem, LWTime time);
	double (*shrinkEdgesNominalDistance)( LWItemID objectitem, LWTime time); /* added for version 5: obtains nominal distance value used when shrink edges is enabled. */
...
#define LWLIGHTINFO_GLOBAL	"LW Light Info 4"
...
#define LWCAMERAINFO_GLOBAL	"LW Camera Info 4"
...
typedef struct st_LWCameraInfo {
	...
	int			(*evaluateRay)	 (LWItemID, double fpx, double fpy, LWCameraRay* camray); /* added for version 4: evaluate the ray at the current time */
} LWCameraInfo;
...
#define LWSCENEINFO_GLOBAL "LW Scene Info 4"
...
#define LWINTERFACEINFO_GLOBAL	"LW Interface Info 5"
