 ...
 */
#ifdef _LWCORE
    // Used internally by the Lightwave core only.
    namespace GCore
    {
        class Vertex;
        class Edge;
    ...
    typedef GCore::Vertex*       LWPntID;
    typedef GCore::Edge*         LWEdgeID;
    typedef GCore::Polygon*      LWPolID;
#else
    // SDK version for third party developers with the original type
...
typedef struct st_LWMeshInfo
{
    ...
    int             (*numEdges)     (LWMeshInfoID);
    size_t          (*scanEdges)    (LWMeshInfoID, LWEdgeScanFunc *, void *);
...
#define LWOBJECTFUNCS_GLOBAL    "Scene Objects 6"
