...
*/
#define LWEVNT_NOTIFY_SURFACE_ADDED 0x120 /// @todo

/**
indicates a surface is about to be removed.
eventData is the surface id that is going to be removed.
*/
#define LWEVNT_NOTIFY_SURFACE_REMOVING 0x121 /// @todo

/// Indicates a value from a plugin instance has changed.
/// Dependencies on that plugin may wish to be updated and
/// can determine if they truely need to be by receiving this event,
/// comparing the plugin class, and slot number.
/// The specific value that changed that caused this event is not
/// pass along however.
/// eventData is a structure that contains the plugin class
/// and instance's slot number.
#define LWEVENT_NOTIFY_PLUGIN_UPDATED 0x122

/// This is the structure passed along as eventData when the
/// LWEVENT_NOTIFY_PLUGIN_UPDATED event is sent.
typedef struct st_LWEventNotifyPluginUpdated
{
  unsigned long classtype;
  unsigned long slot;
} LWEventNotifyPluginUpdated;
